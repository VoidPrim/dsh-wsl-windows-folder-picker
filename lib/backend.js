 ...
 *
 * Exports one factory that builds a `directoryPicker` seam capability of kind
 * `native`: it opens a real Windows folder chooser. On WSL that is the whole
 * point — the shipped adaptive chooser
 * (`dsh-host-directory-picker-auto`) can never reach Windows from Linux, since
 * its Linux branch only drives `zenity`/`kdialog`. This backend instead runs
 * `powershell.exe` through WSL interop and drives the same modern shell dialog
 * (IFileDialog with FOS_PICKFOLDERS) that Explorer uses.
 *
 * The picked Windows path is translated back with `wslpath -u`, so the value
 * handed to the caller is an ordinary WSL path (`/mnt/h/...`), or a Linux path
 * such as `/home/you/proj` when the operator picked a `\\wsl.localhost\...`
 * location.
 *
 * This module deliberately exports a factory rather than a Cordis plugin: the
 * settings toggle in `index.js` mounts exactly one backend at a time, and both
 * choices are created here.
 *
 ...
 *
 * The kernel release string is the authoritative signal and is checked first;
 * the environment markers are consulted too because they are what the profile
 * patch gates on, and the two must agree — if the patch mounts this backend, the
 * backend must take its Windows branch rather than the zenity one. No single
 * marker is always exported, so all three are accepted.
 *
...
/**
 * Locate a PowerShell able to run the chooser script. `powershell.exe` (Windows
 * PowerShell 5.1, present on every Windows install) is preferred because the
 * script uses `-STA`, which the 5.1 host supports directly.
 *
 * Every candidate must be *proven* runnable here, because this function also
 * answers {@link windowsPickerAvailable}: returning an unverifiable command name
 * would report the chooser as available in an environment where interop is
 * broken, and the user would only discover that from a failed pick.
 *
 * WSL is the reason that probe cannot be a plain `PATH` scan. Interop appends the
 * Windows `PATH` to the Linux one, so a bare `powershell.exe` usually resolves —
 * but when interop is disabled the name still satisfies `PATH` probing while
 * `execFile` fails with `ENOENT`. Windows PowerShell is therefore also looked for
 * at its well-known install locations, which answers the question directly.
 *
 * @returns a runnable PowerShell path or command name, or undefined.
 */
...
/**
 * Combine the caller's lifetime with a wall-clock cap.
 *
 * `execFile` accepts one signal, so the cap and the caller's `signal` are
 * joined into a single derived signal: an abort from either source terminates
 * the child. A non-finite or non-positive cap means "no cap", which keeps the
 * caller's signal exactly as it was.
 *
...
/**
 * Build the `directoryPicker` capability served by this backend.
 *
 * The returned object must stay stable for the service lifetime — consumers may
 * capture it across calls — so the remembered starting folder lives in this
 * closure rather than being recreated per pick.
 *
 * `options` is a live reader, not a snapshot: the host capability is consulted
 * on every call, so flipping a preference takes effect on the next pick without
 * remounting anything.
 *
 * @param readOptions - reads `{rememberLast, timeoutSeconds}` from the live preferences.
 ...
 *
 * The browse backend is a cordis `Service` that registers `directoryPicker` in
 * its constructor, so instantiating it would collide with this plugin's own
 * provision. Its listing and creation methods read nothing but `this.config`,
 * so a prototype-only instance runs the exact shipped code — byte for byte the
 * same listing shape, truncation rule, and error codes the in-page browser
 * already consumes — with no duplicated logic and no second service.
 *
 * @param ctx - Host plugin context carrying `baseUrl`.
 * @returns an object exposing the shipped `list` and `createDirectory`.
 */
function loadBrowsePrimitives(ctx) {
  const baseUrl = ctx.baseUrl
  if (typeof baseUrl !== 'string' || baseUrl.length === 0) {
    throw new Error(
      'dsh-wsl-windows-folder-picker: ctx.baseUrl is unavailable, so the in-page directory browser cannot be loaded',
    )
  }
  let loaded
  try {
    const anchored = createRequire(new URL('package.json', baseUrl).href)
    loaded = anchored('@deepseek-ai/dsh-host-directory-picker-browse')
  } catch (cause) {
    // Surfaced with the action that fixes it: this package ships no build step and
    // no dependency of its own, so the browse backend must come from the dsh
    // installation the profile already resolves.
    throw new Error(
      'dsh-wsl-windows-folder-picker: could not load @deepseek-ai/dsh-host-directory-picker-browse from the ' +
        `profile, so the in-page directory browser is unavailable. Reinstall the plugin, or turn the plugin off ` +
        `to let dsh's own picker seat the browser. (${cause instanceof Error ? cause.message : String(cause)})`,
    )
  }
...
/**
 * Build the single capability this plugin serves for both modes.
 *
 * `kind` is read fresh by the workspace controller on every call, so one stable
 * capability can answer `native` while the Windows chooser is wanted and
 * `browse` otherwise. That is what makes the switch live: nothing is mounted or
 * unmounted, and the seam never disagrees with the browser surface reading it —
 * the browser half flips its own slot registration off the same two facts.
 *
 * The rule is deliberately the same one the client half applies
 * (`wantsNativeSurface`): serve `native` unless `available` or `enabled` is
 * explicitly false. Duplicating a two-clause rule is the price of the two halves
 * being unable to share code — the client bundle is a separate program — and
 * keeping them literally identical is what guarantees the mounted surface always
 * matches the served capability.
 *
