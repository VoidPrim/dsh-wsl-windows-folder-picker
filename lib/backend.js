...
function findPowerShell() {
  ...
  for (const candidate of candidates) {
    // A bare command name is resolved by the OS (and by WSL interop), so it only
    // needs the PATH probe; an absolute path is checked directly.
    ...
    if (candidate === 'powershell.exe' || candidate === 'pwsh.exe') {
      // WSL interop resolves Windows executables that are not on the Linux PATH.
      if (isWsl()) return candidate
    }
  }
  return undefined
}
...
async function pickMac(signal) {
  ...
  // osascript exits non-zero when the operator cancels.
  if (/User canceled|-128/i.test(result.stdout)) return { kind: 'cancel' }
  return { kind: 'cancel' }
}
 ...
 */
export function createWindowsCapability(readOptions) {
