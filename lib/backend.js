 ...
 *
 * The remembered folder is always used and the dialog always caps at
 * {@link DEFAULT_TIMEOUT_MS}: both were Config fields once, and with the schema
 * gone there is no longer a way to vary them.
 *
  ...
  return {
    ...
    async pick(signal) {
      ...
      if (isWsl() || process.platform === 'win32') {
        outcome = await pickWindows(signal, lastWindowsPath, DEFAULT_TIMEOUT_MS)
 ...
 *
 * `kind` is probed once at boot rather than stored, because the one fact that
 * decides it — whether interop can run `powershell.exe` — cannot change while
 * the process lives. A boot fact, not a preference: with interop off there is no
 * dialog to open, so `browse` is served whether or not the operator wanted the
 * Windows chooser. The browser half occupies its seats on the same terms (see
 * `lib/client.js`), so the mounted surface and the served capability agree.
 *
 * @param ctx - Host plugin context.
 * @returns the dispatching capability.
 */
export function createDispatchingCapability(ctx) {
  const windows = createWindowsCapability()
  let browse
  const browseFace = () => {
    if (browse === undefined) browse = loadBrowsePrimitives(ctx)
    return browse
  }
  const kind = windowsPickerAvailable() ? 'native' : 'browse'
  return {
    kind,
    async pick(signal) {
