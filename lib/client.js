window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Priority of this plugin's picker surface.
		 *
		 * A `single` slot renders the LIVE entry with the LOWEST priority, and two
		 * entries may not share one. The shipped browse surface registers at the
		 * default 0, so registering at -1 makes this surface the winner whenever it
		 * is mounted, and disposing it hands the seat straight back to browse. That
		 * handoff is the whole live switch on the browser side: no page reload and
		 * no boot-graph change, both of which are impossible at runtime.
		 */
		 ...
		 * seat, given one settings snapshot.
		 *
		 * The two halves of the picker must agree on the mode, and the browser half
		 * cannot read the Host's capability synchronously — so it derives the answer
		 * from the same facts the Host uses, and matches the Host's default whenever
		 * it has no facts yet.
		 *
		 * That default is the plugin's whole point: the Windows chooser. It is also
		 * the only choice that stays consistent in the degraded cases, which are
		 * persistent rather than momentary. When the Host cannot register the
		 * namespace at all — no settings service, or schemastery unreachable — it
		 * still serves its defaults, and the browser observes precisely this: a
		 * namespace that never arrives. Treating that as "browse" would mount the
		 * browse surface against a capability serving `native`, and every listing
		 * would fail.
		 *
		 * So only positive evidence turns the surface off: `available: false` (the
		 * Host cannot serve the Windows chooser here) or `enabled: false` (the user
		 * turned it off). Everything else — a loading mirror, a mirror that never
		 * answers, a served section missing the key — leaves the two halves agreeing
		 * on the same default, because the Host applies that default too.
		 *
		...
		/**
		 * Drive one preference with instant feedback.
		 *
		 * A plain read-through would leave a control dead until the Host round trip
		 * lands — the first press appears to do nothing, which is exactly the bug this
		 * avoids. A change records an optimistic value immediately, and that override
		 * is dropped as soon as the scope reports the same value back, so the control
		 * never fights the authoritative value once it arrives.
		 *
		 * Writes are deliberately NOT gated on `writable`: before the first Host view
		 * arrives `writable` is false precisely because the mirror is still loading,
		 * and an unset revision means "write unconditionally" — which is the correct
		 * thing to do in that window. Gating on `writable` here is what swallows a
		 * user's first click.
		 *
		...
		function WslPickerSection(props) {
			...
			const timeoutPref = usePreference(scope, "timeoutSeconds", 300);
		 ...
		 *
		 *   1. This plugin's own Settings page (`settings.section`), and
		 *   2. the native directory-flow surface, mounted only while the Host
		 *      actually serves the `native` capability.
		 *
		 * The second point is what keeps the two halves in agreement. The Host
		 * publishes `available` (whether powershell.exe was reachable) alongside the
		 * user's `enabled`, so the browser can decide from the same facts the Host
		 * used — and the decision is a slot registration that appears or disappears,
		 * never a module-graph change the running page could not follow.
		 *
